...
def maxflow(bfs_max_iterations=float('inf'), flow_max_iterations=float('inf')):
    ...
    while True:
        ...
        while not sink_path_found:
            ...
            db.execute("""
										SELECT array_cat(p.path, array[e.id]) AS path, array_cat(p.nodes, array[e.dst]) AS nodes
                    INTO tmp
										FROM paths p, edge e
										WHERE e.src=nodes[array_length(nodes,1)] AND e.capacity != 0
        ...
        db.execute("""
            ...
            constraining_capacity(capacity) AS (
                SELECT MIN(e.capacity) AS capacity 
								FROM path_edges pe, edge e
								WHERE e.id=pe.path_edge
            )
        ...
        db.execute("""
            WITH updates(id, new_capacity) AS (
                SELECT f.edge_id AS id, e.capacity - f.flow AS new_capacity
								...
								FROM flow_to_route f, flip_edge fe, edge e
								WHERE e.id=fe.reverse_id AND f.edge_id=fe.forward_id
            )
            UPDATE edge
