...
USCashMap.prototype.render = function (data) {
    ...
    this.states.on('mouseover', function(d, e, p){
        ...
        // Implement
        // NEED TO: get state code ('CA'), change color of that state, setInspectionInfo
        // for that state, and keep rest of states the same
        var state = d['state'];
        var total_amount = d['total_amount'];
 
        that.addStateToSelection(state);
        that.setInspectionInfo(state, total_amount);
        that.states.transition().duration(1)
            .attr("fill", function(d) {
