...
object CS186Utils {
  ...
  def getUdfFromExpressions(expressions: Seq[Expression]): ScalaUdf = {
    // IMPLEMENT ME
		...
		if (expressions.hasDefiniteSize) {
			...
			if (expr != null) {
				result = expr.asInstanceOf[ScalaUdf]
			}
		}
		result
  }
    ...
    { input =>
      new Iterator[Row] {
        ...
        def next() = {
          // IMPLEMENT ME
          if (input.hasNext) {
            var nxt: Row = input.next()
            var pre: Row = preUdfProjection(nxt)
            var post: Row = postUdfProjection(nxt)
            var nextKey: Row = cacheKeyProjection(nxt)
            if (!cache.containsKey(nextKey)) {
              cache.put(nextKey, udfProject(nxt))
            }
            Row.fromSeq(preUdfProjection(nxt) ++ cache.get(nextKey) ++ postUdfProjection(nxt))
