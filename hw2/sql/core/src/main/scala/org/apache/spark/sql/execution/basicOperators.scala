...
case class PartitionProject(projectList: Seq[Expression], child: SparkPlan) extends UnaryNode {
  ...
  def generateIterator(input: Iterator[Row]): Iterator[Row] = {
    ...
    // IMPLEMENT ME
		// partition input to disk here:
		var diskHashedRelation: DiskHashedRelation = DiskHashedRelation.apply(input,keyGenerator)
		var partitions: Iterator[DiskPartition] = diskHashedRelation.getIterator()
    val generator: (Iterator[Row] => Iterator[Row]) = CS186Utils.generateCachingIterator(projectList, child.output)
		var currentIter: Iterator[Row] = null
    var dataLeft: Boolean = true
    new Iterator[Row] {
      def hasNext() = {
        // IMPLEMENT ME
				if (currentIter == null || !(currentIter.hasNext)) {
					dataLeft = fetchNextPartition()
      ...
      def next() = {
        // IMPLEMENT ME
        if (currentIter.hasNext || dataLeft) {
					currentIter.next()
        } else {
          null
        }
      ...
      private def fetchNextPartition(): Boolean  = {
        // IMPLEMENT ME
				var partitionsLeft: Boolean = true
				if (partitions.hasNext) {
					currentIter = generator.apply(partitions.next().getData())
				}	else {
					partitionsLeft = false
					diskHashedRelation.closeAllPartitions()
				}
				partitionsLeft
      }
    }
